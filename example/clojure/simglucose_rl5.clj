...
;;   simglucose_rl5.clj
;; DESCRIPTION
;;   Verify STL properties on the PPO-controlled simglucose Python environment (case 5)
;; AUTHOR
;;   Masaki Waga
;; HISTORY
...
;;   lein exec -p simglucose_rl5.clj
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(def stl-list
  (let [minutes-to-steps (fn [minutes] (int (/ minutes step-duration)))]
    (parse-stl-list
     [;; BG should not stay below 10th-percentile threshold for more than 60 minutes
      (format "G(%s > %s || F_[0, %d] %s > %s)" bg tenth-bg (minutes-to-steps 60) max-bg tenth-bg)
      ;; BG should not stay above 90th-percentile threshold for more than 60 minutes
      (format "G(%s < %s || F_[0, %d] %s < %s)" bg ninetieth-bg (minutes-to-steps 60) min-bg ninetieth-bg)
      ;; BG should not stay above 90th-percentile threshold for more than 30 minutes after an insulin injection
      (format "G(%s < %s || %s < 0.5 || F_[0, %d] %s < %s)" bg ninetieth-bg insulin (minutes-to-steps 30) min-bg ninetieth-bg)]
     input-mapper
